...
export default {
  ...
  setup(props, { emit }) {
    ...
    const { grnDetails: detailsResource, rejectGRN: rejectResource } = useGRN('nandi-hills')
    ...
    const approveGRN = async () => {
      // Nandi Hills posts directly to SAP - no separate approval step
      alert('GRNs in Nandi Hills are posted directly to SAP upon creation')
      emit('close')
    }
