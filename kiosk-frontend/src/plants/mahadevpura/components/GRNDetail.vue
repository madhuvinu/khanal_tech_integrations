...
export default {
  ...
  setup(props, { emit }) {
    ...
    const { grnDetails: detailsResource, rejectGRN: rejectResource } = useGRN('mahadevpura')
    ...
    const approveGRN = async () => {
      // Mahadevpura posts directly to SAP - no separate approval step
      alert('GRNs in Mahadevpura are posted directly to SAP upon creation')
      emit('close')
    }
