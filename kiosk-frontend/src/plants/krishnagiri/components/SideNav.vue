      ...
      <div class="p-4 border-b border-gray-200">
        <div class="flex items-center space-x-3">
          ...
          <div v-if="!isCollapsed" class="flex-1 min-w-0">
            <h2 class="text-sm font-semibold text-gray-900 truncate">Krishnagiri</h2>
            <p class="text-xs text-gray-500 truncate">Regional Production</p>
          </div>
        </div>
      </div>
...
// Navigation items for Krishnagiri
const navigationItems = ref([
  {
    id: 'dashboard',
    name: 'Dashboard',
    icon: '📊',
    route: '/plant/krishnagiri/dashboard'
  },
  {
    id: 'grn',
    name: 'GRN',
    icon: '📦',
    route: '/plant/krishnagiri/grn'
  },
  {
    id: 'production-order',
    name: 'Production Order',
    icon: '🏭',
    route: '/plant/krishnagiri/production-order'
  },
  {
    id: 'quality',
    name: 'Quality',
    icon: '✅',
    route: '/plant/krishnagiri/quality'
  },
  {
    id: 'inventory-transfer',
    name: 'Inventory Transfer',
    icon: '🔄',
    route: '/plant/krishnagiri/inventory-transfer'
  }
])
...
const currentPageTitle = computed(() => {
  ...
  return currentItem ? currentItem.name : 'Krishnagiri Plant'
})
...
const saveNavigationState = () => {
  ...
  localStorage.setItem('krishnagiri-nav-state', JSON.stringify(state))
}

const loadNavigationState = () => {
  try {
    const saved = localStorage.getItem('krishnagiri-nav-state')
    if (saved) {
      const state = JSON.parse(saved)
      if (state.plant === 'krishnagiri') {
