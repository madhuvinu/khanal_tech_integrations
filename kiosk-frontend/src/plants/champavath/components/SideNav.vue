      ...
      <div class="p-4 border-b border-gray-200">
        <div class="flex items-center space-x-3">
          ...
          <div v-if="!isCollapsed" class="flex-1 min-w-0">
            <h2 class="text-sm font-semibold text-gray-900 truncate">Champavath</h2>
            <p class="text-xs text-gray-500 truncate">Specialized Processing</p>
          </div>
        </div>
      </div>
...
// Navigation items for Champavath
const navigationItems = ref([
  {
    id: 'dashboard',
    name: 'Dashboard',
    icon: '📊',
    route: '/plant/champavath/dashboard'
  },
  {
    id: 'grn',
    name: 'GRN',
    icon: '📦',
    route: '/plant/champavath/grn'
  },
  {
    id: 'production-order',
    name: 'Production Order',
    icon: '🏭',
    route: '/plant/champavath/production-order'
  },
  {
    id: 'inventory-transfer',
    name: 'Inventory Transfer',
    icon: '🔄',
    route: '/plant/champavath/inventory-transfer'
  }
])
...
const currentPageTitle = computed(() => {
  ...
  return currentItem ? currentItem.name : 'Champavath Plant'
})
...
const saveNavigationState = () => {
  ...
  localStorage.setItem('champavath-nav-state', JSON.stringify(state))
}

const loadNavigationState = () => {
  try {
    const saved = localStorage.getItem('champavath-nav-state')
    if (saved) {
      const state = JSON.parse(saved)
      if (state.plant === 'champavath') {
