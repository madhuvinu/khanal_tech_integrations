      ...
      <div class="p-4 border-b border-gray-200">
        <div class="flex items-center space-x-3">
          ...
          <div v-if="!isCollapsed" class="flex-1 min-w-0">
            <h2 class="text-sm font-semibold text-gray-900 truncate">Malur</h2>
            <p class="text-xs text-gray-500 truncate">Processing Facility</p>
          </div>
        </div>
      </div>
...
// Navigation items for Malur (limited set)
const navigationItems = ref([
  {
    id: 'dashboard',
    name: 'Dashboard',
    icon: '📊',
    route: '/plant/malur/dashboard'
  },
  {
    id: 'grn',
    name: 'GRN Management',
    icon: '📦',
    route: '/plant/malur/grn'
  },
  {
    id: 'production-order',
    name: 'Production Order',
    icon: '🏭',
    route: '/plant/malur/production-order'
  },
  {
    id: 'inventory-transfer',
    name: 'Inventory Transfer',
    icon: '🔄',
    route: '/plant/malur/inventory-transfer'
  }
])
...
const currentPageTitle = computed(() => {
  ...
  return currentItem ? currentItem.name : 'Malur Plant'
})
...
const saveNavigationState = () => {
  ...
  localStorage.setItem('malur-nav-state', JSON.stringify(state))
}

const loadNavigationState = () => {
  try {
    const saved = localStorage.getItem('malur-nav-state')
    if (saved) {
      const state = JSON.parse(saved)
      if (state.plant === 'malur') {
