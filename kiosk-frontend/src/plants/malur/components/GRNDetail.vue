<template>
  <div class="space-y-6">
    ...
    <div v-else-if="grnDetails">
      ...
      <!-- Line Items -->
      <div class="bg-gray-50 p-6 rounded-lg">
        <h3 class="text-lg font-semibold mb-4 text-gray-900">Received Items</h3>
          ...
          <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-100">
              <tr>
                ...
                <th class="px-4 py-3 text-right text-xs font-semibold text-gray-700 uppercase">Ordered Qty</th>
                <th class="px-4 py-3 text-right text-xs font-semibold text-gray-700 uppercase">Received Qty</th>
                <th class="px-4 py-3 text-left text-xs font-semibold text-gray-700 uppercase">Warehouse</th>
                <th class="px-4 py-3 text-left text-xs font-semibold text-gray-700 uppercase">Batches</th>
              </tr>
            </thead>
            <tbody class="bg-white divide-y divide-gray-200">
              <template v-for="(item, index) in grnDetails.line_items" :key="index">
                <tr>
                  ...
                  <td class="px-4 py-3 text-sm text-gray-700">{{ item.ItemDescription }}</td>
                  <td class="px-4 py-3 text-sm text-right text-gray-700 font-medium">{{ item.OrderedQuantity }}</td>
                  <td class="px-4 py-3 text-sm text-right font-semibold text-gray-900">{{ item.ReceivedQuantity }}</td>
                  <td class="px-4 py-3 text-sm text-gray-700">{{ item.WarehouseCode }}</td>
    ...
    <div v-if="showRejectDialog" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div class="bg-white rounded-lg p-6 w-96">
        <h3 class="text-lg font-bold mb-4 text-gray-900">Reject GRN</h3>
        <textarea
          v-model="rejectionReason"
          placeholder="Enter rejection reason..."
          class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 bg-white text-gray-900"
          rows="4"
        ></textarea>
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const { grnDetails: detailsResource, rejectGRN: rejectResource } = useGRN('malur')
    ...
    const loadGRNDetails = async () => {
      ...
      try {
        ...
        if (detailsResource.data?.success && detailsResource.data?.data) {
          ...
          if (data.line_items && Array.isArray(data.line_items)) {
            data.line_items = data.line_items.map(item => ({
              ...
              BatchLines: item.batch_lines || [],
              // Keep original snake_case too
              showBatches: false
            }))
            
            // Transform batch lines
            data.line_items.forEach(item => {
              if (item.BatchLines && Array.isArray(item.BatchLines)) {
                item.BatchLines = item.BatchLines.map(batch => ({
                  ...batch,
                  BatchNumber: batch.batch_number,
                  Quantity: batch.quantity,
                  MoistureValue: batch.moisture_value || 0
                }))
              }
            })
          }
        ...
        const errorMsg = error.response?.data?.message || error.message || 'Failed to load GRN details'
        alert(`❌ Error loading GRN details:\n\n${errorMsg}\n\nGRN ID: ${props.grnId}`)
        // Go back to list after error
        emit('close')
      } finally {
        loading.value = false
      }
    ...
    const approveGRN = async () => {
      // Malur posts directly to SAP - no separate approval step
      alert('GRNs in Malur are posted directly to SAP upon creation')
      emit('close')
    }
