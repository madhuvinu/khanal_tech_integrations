...
def current_month_processed_milk_data():
    try:
        result = frappe.db.sql("""
            SELECT 
                sap_data_row AS 'current_month_processed_milk_data'
            FROM
                taball_custom_reports_data
            WHERE
                report_name = 'Monthly_WorkOrders_Data_per_WareHouse'
                AND JSON_UNQUOTE(JSON_EXTRACT(sap_data_row, '$.Year-Month')) = DATE_FORMAT(CURDATE(), '%Y-%m-%b')
            ORDER BY creation DESC;
        """, as_dict=True)
        return [json.loads(r['current_month_processed_milk_data']) for r in result]
