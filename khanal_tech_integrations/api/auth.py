...
def verify_plant_access(user, plant_id):
    ...
    try:
        ...
        return {
            ...
            "user_access": user_plant_access[0]
        }
...
def get_user_permissions(user, plant_id):
    ...
    try:
        ...
        # Parse permissions
        permissions = []
        if user_plant_access[0].permissions:
            permissions = json.loads(user_plant_access[0].permissions)
        
        # Add role-based permissions
        role = user_plant_access[0].role
        if role == "Admin":
            permissions.extend(["admin", "grn", "crate", "production_order", "department", "settings"])
        elif role == "Manager":
            permissions.extend(["grn", "crate", "production_order", "department"])
        elif role == "Operator":
            permissions.extend(["grn", "crate"])
        elif role == "Viewer":
            permissions.extend(["view_only"])
...
def generate_plant_token(user, plant_id, permissions):
    ...
    try:
        ...
        log_authentication_activity(user, plant_id, "login_success", {
...
def reset_password_with_token(token, new_password):
    ...
    try:
        ...
        # Log activity (plant optional here)
        try:
            log_authentication_activity(user_email, data.get("plant_id"), "password_reset", {"via": "token"})
        except Exception:
            pass
...
def change_password(current_password, new_password, plant_id):
    ...
    try:
        ...
        # Log password change
        log_authentication_activity(user.name, plant_id, "password_change", {
            "user_agent": frappe.get_request_header("User-Agent"),
            "ip_address": frappe.get_request_header("X-Forwarded-For") or frappe.local.request_ip
        })
...
def log_authentication_activity(user, plant_id, activity_type, metadata=None):
    """
    ...
    """
    try:
        # Check if Kiosk Activity Log doctype exists
        if not frappe.db.exists("DocType", "Kiosk Activity Log"):
            frappe.log_error("Kiosk Activity Log doctype not found - skipping activity logging")
            return
        ...
        frappe.log_error(f"Error logging authentication activity: {str(e)}")
        # Don't throw the error - just log it and continue

@frappe.whitelist(allow_guest=True)
def setup_admin_plant_access():
    """
    Setup Administrator access to Mahadevpura plant
    This is a one-time setup function
    """
    try:
        # Check if Administrator already has access
        existing_access = frappe.get_all(
            "User Plant Access",
            filters={
                "user": "Administrator",
                "plant": "Mahadevpura",
        ...
        if existing_access:
            return {"message": "Administrator already has access to Mahadevpura plant"}
        
        # Create User Plant Access record
        user_plant_access = frappe.get_doc({
            "doctype": "User Plant Access",
            "user": "Administrator",
            "plant": "Mahadevpura",
            "role": "Admin",
            "permissions": '["dashboard", "grn", "production-order", "quality", "inventory-transfer", "admin"]',
            "enabled": 1
        })
        
        user_plant_access.insert(ignore_permissions=True)
        frappe.db.commit()
        
        return {
            "message": "Administrator access to Mahadevpura plant created successfully",
            "record_id": user_plant_access.name
        }
        
    except Exception as e:
        frappe.log_error(f"Error setting up admin plant access: {str(e)}")
        return {"error": str(e)}
...
def get_user_plants(user):
    ...
    try:
        ...
        for user_plant in user_plants:
            plant_doc = frappe.get_doc("Plant", user_plant.plant)
            if plant_doc.status == "Active":
                plants.append({
                    "id": plant_doc.name,
                    "name": plant_doc.plant_name,
