"""
Krishnagiri Plant Production APIs
Handles plant-specific Production operations for Krishnagiri plant
...
class KrishnagiriProductionAPI(CommonProductionAPI):
    """
    Krishnagiri Plant Production API Implementation
    ...
    # Override them here ONLY if Krishnagiri needs custom behavior
...
def search_bom(search_query, plant_id='krishnagiri'):
...
@frappe.whitelist(allow_guest=False)
def get_itt1_components(bom_code, plant_id='krishnagiri'):
...
@frappe.whitelist(allow_guest=False)
def get_oitt_header(bom_code, plant_id='krishnagiri'):
...
@frappe.whitelist(allow_guest=False)
def get_batch_numbers(item_code, plant_id='krishnagiri', warehouse=None, date_from=None, date_to=None):
...
@frappe.whitelist(allow_guest=False)
def get_batch_numbers_from_batch_date_item(item_code, date_from=None, date_to=None, plant_id='krishnagiri'):
...
@frappe.whitelist(allow_guest=False)
def get_warehouses(plant_id='krishnagiri'):
...
@frappe.whitelist(allow_guest=False)
def approve_production_order(production_data, plant_id='krishnagiri'):
...
@frappe.whitelist(allow_guest=False)
def goods_issue(production_order_doc_entry, production_data, plant_id='krishnagiri'):
...
@frappe.whitelist(allow_guest=False)
def goods_receipt(production_order_doc_entry, production_data, plant_id='krishnagiri'):
...
@frappe.whitelist(allow_guest=False)
def close_production(production_order_doc_entry, close_date=None, plant_id='krishnagiri'):
...
@frappe.whitelist(allow_guest=False)
def get_production_orders_list(filters=None, page=1, page_size=20, plant_id='krishnagiri'):
...
@frappe.whitelist(allow_guest=False)
def get_production_order_for_resume(production_kiosk_name, plant_id='krishnagiri'):
