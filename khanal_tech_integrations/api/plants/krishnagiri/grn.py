"""
Krishnagiri Plant GRN APIs
Handles all GRN operations for Krishnagiri plant
"""
...
class KrishnagiriGRNAPI(BaseGRNAPI):
    """Krishnagiri Plant GRN API Implementation"""
    
    def __init__(self):
        super().__init__('krishnagiri')
    
    def get_warehouse_filter(self, table_alias='POR1'):
        """
        Override warehouse filter for Krishnagiri
        Warehouses start with 'KG' or 'DK'
        """
        return f"AND ({table_alias}.\"WhsCode\" LIKE 'KG%' OR {table_alias}.\"WhsCode\" LIKE 'DK%')"
...
@frappe.whitelist()
def get_purchase_orders(plant_id='krishnagiri', status='Open', from_date=None, to_date=None, fetch_all=False, bp_search=None):
    """
    Get open purchase orders for Krishnagiri plant with optional filters
    
    Args:
        plant_id (str): Plant identifier (default: krishnagiri)
...
@frappe.whitelist()
def get_po_line_items(doc_entry, plant_id='krishnagiri'):
    """
...
def create_grn_draft(grn_data):
    ...
    (Krishnagiri plant: No draft approval, direct SAP posting)
    ...
    try:
        ...
        grn_doc = frappe.get_doc({
            ...
            "type": "krishnagiri",  # ✅ Store plant_id in type field for filtering
        ...
        frappe.log_error(f"Error creating GRN: {str(e)}", "Krishnagiri GRN Creation Error")
        return {
            "success": False,
            "message": str(e)
        }
...
@frappe.whitelist()
def get_grn_list(filters=None, page=1, page_size=20, plant_id='krishnagiri'):
    """
    ...
    try:
        ...
        frappe.log_error(f"Error fetching GRN list: {str(e)}", "Krishnagiri GRN List Error")
        return {
            "success": False,
            "message": str(e),
            "data": []
...
@frappe.whitelist()
def get_grn_details(grn_id, plant_id='krishnagiri'):
    """
    ...
    try:
        ...
        frappe.log_error(f"Error fetching GRN details: {str(e)}", "Krishnagiri GRN Details Error")
        return {
            "success": False,
            "message": str(e)
        }
...
def send_grn_notification(grn_doc, sap_result):
    ...
    """
    try:
        # Krishnagiri-specific notification recipients
        recipients = [
            "krishnagiri.plant@khanalfoods.com",  # Update with actual email
            "operations@khanalfoods.com"
        ]
        
        subject = f"GRN Created - Krishnagiri Plant - SAP DocNum: {sap_result.get('doc_num')}"
        message = f"""
        <h3>GRN Successfully Posted to SAP</h3>
        <p><strong>Plant:</strong> Krishnagiri</p>
        ...
        frappe.log_error(f"Error sending GRN notification: {str(e)}", "Krishnagiri GRN Notification Error")


@frappe.whitelist(methods=['POST'])
def cancel_grn(grn_id, reason, plant_id='krishnagiri'):
    """
