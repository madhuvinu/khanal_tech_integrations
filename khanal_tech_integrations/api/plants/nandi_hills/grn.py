"""
Nandi Hills Plant GRN APIs
Handles all GRN operations for Nandi Hills plant
"""
...
class NandiHillsGRNAPI(BaseGRNAPI):
    """Nandi Hills Plant GRN API Implementation"""
    ...
    def get_warehouse_filter(self, table_alias='POR1'):
        """
        Override warehouse filter for Nandi Hills
        Warehouses start with 'NH'
        """
        return f"AND {table_alias}.\"WhsCode\" LIKE 'NH%'"
...
@frappe.whitelist()
def get_purchase_orders(plant_id='nandi_hills', status='Open', from_date=None, to_date=None, fetch_all=False, bp_search=None):
    """
    Get open purchase orders for Nandi Hills plant with optional filters
...
@frappe.whitelist()
def get_po_line_items(doc_entry, plant_id='nandi_hills'):
    """
...
def create_grn_draft(grn_data):
    ...
    (Nandi Hills plant: No draft approval, direct SAP posting)
    ...
    try:
        ...
        grn_doc = frappe.get_doc({
            ...
            "type": "nandi_hills",  # ✅ Store plant_id in type field for filtering
        ...
        frappe.log_error(f"Error creating GRN: {str(e)}", "Nandi Hills GRN Creation Error")
        return {
            "success": False,
            "message": str(e)
        }
...
@frappe.whitelist()
def get_grn_list(filters=None, page=1, page_size=20, plant_id='nandi_hills'):
    """
    ...
    try:
        ...
        frappe.log_error(f"Error fetching GRN list: {str(e)}", "Nandi Hills GRN List Error")
        return {
            "success": False,
            "message": str(e),
            "data": []
...
@frappe.whitelist()
def get_grn_details(grn_id, plant_id='nandi_hills'):
    """
    ...
    try:
        ...
        frappe.log_error(f"Error fetching GRN details: {str(e)}", "Nandi Hills GRN Details Error")
        return {
            "success": False,
            "message": str(e)
        }
...
def send_grn_notification(grn_doc, sap_result):
    ...
    """
    try:
        # Nandi Hills-specific notification recipients
        recipients = [
            "nandihills.plant@khanalfoods.com",  # Update with actual email
            "operations@khanalfoods.com"
        ]
        
        subject = f"GRN Created - Nandi Hills Plant - SAP DocNum: {sap_result.get('doc_num')}"
        message = f"""
        <h3>GRN Successfully Posted to SAP</h3>
        <p><strong>Plant:</strong> Nandi Hills</p>
        ...
        frappe.log_error(f"Error sending GRN notification: {str(e)}", "Nandi Hills GRN Notification Error")


@frappe.whitelist(methods=['POST'])
def cancel_grn(grn_id, reason, plant_id='nandi_hills'):
    """
