...
$(document).ready(function () {
  ...
  var orderTableRanges = $("#order-table-range");

  if (orderTableRanges.length != 0) {
    var startDate_url = GetURLParameter('startDate');
    var endDate_url = GetURLParameter('endDate');
    console.log(startDate_url);

    if (typeof startDate_url !== "undefined") {
      var start = moment(startDate_url);
      var end = moment(endDate_url);
      console.log(start);
    }
    else {
    ...
    function cb(start, end) {
      $("#order-table-range .date-holder").html(
        start.format("MMMM D, YYYY") + " - " + end.format("MMMM D, YYYY")
      );
    }

    orderTableRanges.daterangepicker(
      {
        startDate: start,
        endDate: end,
        opens: "left",
  ...
  // MAKING THE CURRENT NAVIGATION ACTIVE
  $(function () {
    ...
    $('.sidebar-left li a').each(function () {
      ...
      if ($this.attr('href').indexOf(current) !== -1) {
        ...
        $this.parent().parent().parent().parent().addClass('active expand');
      }
    })
  })

  $('#order-table-range').on('apply.daterangepicker', function (ev, picker) {
    var startDate = picker.startDate.format('YYYY-MM-DD');
    var endDate = picker.endDate.format('YYYY-MM-DD');

    var url = "/intranet2/orders?startDate=" + startDate + "&endDate=" + endDate;
...
function GetURLParameter(sParam) {
  ...
  var sURLVariables = decodeURI(sPageURL).split('&');
