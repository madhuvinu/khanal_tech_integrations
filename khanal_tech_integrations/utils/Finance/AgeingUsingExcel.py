...
#! bench --site dev.localhost execute  --args "( '2022-01-01','2023-09-01' )"  khanal_tech_integrations.utils.Finance.AgeingReport.Get_MasterData
table_css_style = """
            <style>
            body {
...
# bench --site dev.localhost execute  --args "( '2022-01-01','2023-09-01','2023-08-25' )"  khanal_tech_integrations.utils.Finance.AgeingReport.Get_MasterData

@frappe.whitelist()
def weekly_Report():
    Today = frappe.utils.nowdate()
    FilterDate = add_to_date(Today,days=-7)
    Get_MasterData(StartDate='2022-01-01',CurrentDate=Today,PreviousDate=FilterDate)

    pass


@frappe.whitelist()
def Get_MasterData(StartDate,CurrentDate,PreviousDate):
    # print(StartDate,CurrentDate,PreviousDate,'\n\n\n\n')
    masterDf=Make_AgeingReport(StartDate,CurrentDate)
    ...
    Vendor_Consolidated=Master_Consolidated(masterDf['VendorData'])
    Customer_Consolidated=Master_Consolidated(masterDf['CustomerData'])

    # !-------------------------------------
    # !? Previous Weak DATA 

    previousmasterDf=Make_AgeingReport(StartDate,PreviousDate)
    ...
    EmailTemplate(CommonTabletructure)



def Make_AgeingReport(start_date,end_date):
    ...
    filtered_df = df[(df['ReferenceDate'] >= start_date) &  (df['ReferenceDate'] <= end_date)]
...
def AdvancePayment(VendorData_df):
    ...
    # Calculate percentage contribution of each 'Outstanding Amount' to the total
    ...
    total_row = pd.DataFrame({'Bucket': ['Total'], 'Outstanding Amount': [total_outstanding_amount], 'Customer/Vendor Code': [total_customers]})
    ...
    # print(AdvancePayment_df,'\n\n\n\n')

   
    # Format negative values with parentheses
 
    cols_to_format = AdvancePayment_df.columns[~AdvancePayment_df.columns.isin(['Bucket'])]
    AdvancePayment_df[cols_to_format] = AdvancePayment_df[cols_to_format].applymap(format_negative)

    # AdvancePayment_df = AdvancePayment_df.applymap(format_negativeinAdv_payment)
...
def GetTotalSum(PayablesData_df,Receivables_df):
    ...
    # print(sum_df,'sum_df','\n\n')
    # cols_to_format = sum_df.columns[~sum_df.columns.isin(['','% of Total Adv Payments','Bucket',])]
...
def Master_Consolidated(Commondf_1cr):
    ...
    html_content=f"""{table_css_style}
        <table >
            ...
            <div style="display: flex; justify-content: space-between;">
                <div>
                    <h1>>1CR vendors wrt pending payments</h1>
                    {Vendor_Consolidated_html_table}
                </div>
                <div>
                    <h1>>1CR customers wrt receiving payments</h1>
...
@frappe.whitelist()
def EmailTemplate(Tabletructure):
    ...
    email_args={
            "recipients":['shahil@khanalfoods.com','harsha@khanalfoods.com'],
            "message":rendered_message,
            "cc": ['yogesha@khanalfoods.com','mratyunjay@khanalfoods.com '],
            "subject":"AP/AR, Advance Payments and Non Banking integration Payments Weekly Reports."
            }
