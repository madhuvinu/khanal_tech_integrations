...
def send_b2b_data_to_sap(docname):
    ...
    try:
        doc = frappe.get_doc("Bigbasket", docname)
        
        if not frappe.has_permission("Bigbasket", "write", doc=doc):
        ...
        if result.get("status") == "success":
            frappe.db.set_value("Bigbasket", doc.name, {
...
def process_sap_integration(doc):
    try:
        ...
        for item in doc.bigbasket_table:
            line_item = {
...
def handle_error(doc, error_msg):
    ...
    frappe.db.set_value("Bigbasket", doc.name, {"error_status": error_msg})
...
def get_total_amount():
    """Calculate the total amount of all purchase orders in Bigbasket using Frappe ."""
