    ...
    <Dialog v-model="show" :options="{
      ...
      <!-- <template #body-title>
        <div class="flex items-center gap-3">
          ...
          <Button v-if="_note?.reference_docname" variant="outline" size="sm" :label="_note.reference_doctype == 'CRM Deal'
              ? __('Open Deal')
              : __('Open Lead')
            " @click="redirect()">
            <template #suffix>
      ...
      <template #body-content>
        <div class="flex flex-col gap-4">
          ...
          <div>
            <div class="mb-1.5 text-sm text-gray-600">{{ __('Title') }}</div>
            <TextInput ref="sub_title" variant="outline" v-model="_note.sub_title" :placeholder="__('Call with John Doe')" />
          </div>
          <div>
...
//   })
  ...
  //     props.note.sub_title === _note.value.sub_title
  //   )
  //     return
